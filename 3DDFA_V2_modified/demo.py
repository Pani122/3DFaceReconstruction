...
def generate_3d_mask(landmarks, image_shape,wfp):
    # Create a 3D array to represent the mask
    mask_3d = np.zeros(image_shape, dtype=np.uint8)

    # Convert 3D landmarks to integer coordinates
    landmarks = landmarks.astype(int)
    # print(landmarks)
    # print(image_shape)
    for landmark in landmarks:
        if(landmark[0] < 1 or landmark[1] < 1 or landmark[2] < 1):
            continue
        if(landmark[0] > 192 or landmark[1] > 192 or landmark[2] > 200):
            continue
        mask_3d[landmark[0]-1][landmark[1]-1][landmark[2]-1] = 1
    np.save(wfp,mask_3d)
    # Create a Poly3DCollection to represent the filled region in 3D
    # polygons = [Poly3DCollection([landmarks], alpha=1.0, linewidths=0.2)]

    # Plot the Poly3DCollection on a 3D axis
...
def main(args):
    ...
    wfp = f'../outputs/{args.img_fp.split("/")[-1].replace(old_suffix, "")}' + ".npy"

    ver_lst = tddfa.recon_vers(param_lst, roi_box_lst, dense_flag=dense_flag)
    ver_lst = np.array(ver_lst)
    
    # print(ver_lst[0].T.shape)
    # print((roi_box_lst))
    if args.opt == '2d_sparse':
        ...
        image_shape = (192,192, 200)
        landmarks_3d = ver_lst[0].T
        size = landmarks_3d.shape[0]
        generate_3d_mask(landmarks_3d, image_shape,wfp)
        
        # render(img, ver_lst, tddfa.tri, alpha=0.6, show_flag=0, wfp=wfp)
    elif args.opt == 'depth':
