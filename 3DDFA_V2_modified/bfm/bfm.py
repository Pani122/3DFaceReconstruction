...
class BFMModel(object):
    def __init__(self, bfm_fp, shape_dim=40, exp_dim=10):
        ...
        self.keypoints = bfm.get('keypoints').astype(np.float32)  # fix bug
        w = np.concatenate((self.w_shp, self.w_exp), axis=1)
        self.w_norm = np.linalg.norm(w, axis=0)

        self.u_base = self.u[self.keypoints.astype(int)].reshape(-1, 1)
        self.w_shp_base = self.w_shp[self.keypoints.astype(int)]
        self.w_exp_base = self.w_exp[self.keypoints.astype(int)]
